...
export default function App() {
    // Smart default view: Kids tab if no children, Log tab if they have children
    const [currentView, setCurrentView] = useState(() => {
        const savedChildren = getStorageData('mybookmark_children', []);
        return savedChildren.length === 0 ? 'kids' : 'log';
    });
    ...
    const [showWelcome, setShowWelcome] = useState(() => {
        return !localStorage.getItem('mybookmark_welcomed');
    });
    const [error, setError] = useState(null);

    const dismissWelcome = () => {
        localStorage.setItem('mybookmark_welcomed', 'true');
    ...
    const addLog = (childId, bookTitle, minutes, date, subject, genre) => {
        // Validation
        if (!childId) {
            ...
            <div className="max-w-2xl mx-auto bg-white min-h-screen shadow-xl">
                ...
                <div className="bg-gradient-to-br from-purple-600 to-purple-800 text-white p-6 text-center">
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <h1 className="text-2xl font-semibold flex-1">🔖 MyBookmark</h1>
                ...
                {showWelcome && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-5 z-50" onClick={dismissWelcome}>
                        <div className="bg-white rounded-2xl p-8 max-w-md w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                            <div className="text-center">
                                <div className="text-5xl mb-4">🔖</div>
                                <h2 className="text-2xl font-bold text-purple-800 mb-6">The My Bookmark Story</h2>
                                ...
                                <div className="text-gray-600 space-y-4 text-left">
                                    ...
                                    <p>
                                        My Bookmark helps you mark every moment,<br />
                                        track every page,<br />
                                        and bookmark the memories that matter.
                                    </p>
                                </div>
                                
                                <div className="mt-8 pt-6 border-t border-gray-100">
                                    <p className="font-semibold text-purple-700 mb-1">Welcome to My Bookmark.</p>
                                    <p className="text-sm text-gray-500 mb-6">Let's start bookmarking your family's reading journey. 🔖</p>
                    ...
                    {recentLogs.map(log => {
                        ...
                        return (
                            <div key={log.id} className="bg-white border border-gray-200 rounded-xl p-4 mb-3 border-l-4 border-l-purple-600 hover:shadow-md transition-shadow">
                                ...
                                <div className="mt-2">
                                    <div className="text-sm text-gray-600 mb-2">{log.bookTitle}</div>
...
function ProgressView({ children, logs, selectedChild, onSelectChild, updateChildGoal, onGenerateReport }) {
...
function AddLogModal({ children, logs, onClose, onAdd }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (selectedChildId && bookTitle.trim() && minutes) {
            onAdd(selectedChildId, bookTitle.trim(), minutes, date);
        }
    };
